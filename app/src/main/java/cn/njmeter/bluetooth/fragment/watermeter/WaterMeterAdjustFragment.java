...
public class WaterMeterAdjustFragment extends BaseFragment implements View.OnClickListener {
    ...
    private static String productType = ProductType.WATER_METER;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment_watermeter_adjust_bluetooth, container, false);
        context = getContext();
        initView(view);
        flowUnit_positive = 0;
        flowUnit_reserve = 0;
    ...
    public void initView(View view) {
        ...
        et_adjust = (EditText) view.findViewById(R.id.et_adjust);
        ...
        btn_adjust = (Button) view.findViewById(R.id.btn_adjust);
    ...
    private void SetAdjust() {
        String x = et_adjust.getText().toString();
        int xi;
        try {
            xi = Integer.parseInt(x);
            if (xi < 500 || xi > 1500) {
                CommonUtils.showToast(context, "请正确输入调整值(500≤X≤1500)");
                return;
            }
        } catch (Exception e) {
            CommonUtils.showToast(context, "请正确输入调整值(500≤X≤1500)");
    ...
    public String adjust(String meterId, String productType, String factoryCode, int x) {
        String r = "";
        try {
            String rx;
            rx = addZeroForNum(Integer.toHexString(x), 4);
            rx = rx.substring(2, 4) + rx.substring(0, 2);
            String cs = AnalysisUtils.getCSSum("68" + productType + meterId + factoryCode + "CD04901F" + rx, 0);
            r = ("68" + productType + meterId + factoryCode + "CD04901F" + rx + cs + "16").toUpperCase();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return r;
