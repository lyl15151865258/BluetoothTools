...
public class BluetoothAnalysisUtils {
     ...
     * 获取校验和
     *
     * @param s     需要计算校验和的16进制字符串
     * @param start 从第start位开始计算
     * @return 字符串类型的CS校验和
     */
    public String getChecksum(String s, int start) {
        int sumL = 0;
        for (int i = (start * 2); i < s.length(); i = i + 2) {
            sumL = (sumL + HexS2ToInt(s.substring(i, i + 2))) % 256;
        }
        return (Integer.toHexString(sumL / 16) + Integer.toHexString(sumL % 16)).toUpperCase();
    }
     ...
     * @param s         需要计算校验和的16进制字符串
     * @param start     从第start位开始计算
     * @param checksumS CS校验和
     * @return boolean类型
     */
    public static boolean checksum(String s, int start, String checksumS) {
        int sumL = 0;
        for (int i = (start * 2); i < s.length(); i = i + 2) {
            sumL = (sumL + HexS2ToInt(s.substring(i, i + 2))) % 256;
        }
        return ((Integer.toHexString(sumL / 16) + Integer.toHexString(sumL % 16)).toUpperCase().equals(checksumS));
    }

    /**
     * 计量单位标准化
     *
     * @param protocol 水表、热表读取值实体类
     * @return 整型
     */
    public static int dataUnitToStandard(Protocol protocol) {
        ...
        try {
            ...
            switch (protocol.getSumHeatUnit()) {
                case "W*h":
            ...
            switch (protocol.getOppositeTotalUnit()) {
                case "mL":
            ...
            switch (protocol.getFlowRateUnit()) {
                case "mL/h":
     ...
     * @param protocol 表数据实体类
     * @return 0或1
     */
    private static int analysisWaterMeterData(String Data, Protocol protocol) {
        int r;
        initProtocol(protocol);
        protocol.setProductTypeTX(Data.substring(2, 2 + 2));
        protocol.setMeterID(Data.substring(4, 4 + 8));
     ...
     * @return 0或1
     */
    private static int analysisWaterMeterData1(String Data, Protocol protocol) {
        int r;
        initProtocol(protocol);
        protocol.setProductTypeTX(Data.substring(2, 2 + 2));
        protocol.setMeterID(Data.substring(4, 4 + 8));
        ...
        protocol.setTimeInP(timeinpS);
        String statusb = AnalysisUtils.HexS2ToBinS(DataArea.substring(50, 50 + 2));
        String status = "";
        char a[] = statusb.toCharArray();
        if (a[2] == '1') {
            status = status + "低电";
     ...
     * @return 0或1
     */
    private static int analysisHydrantData(String Data, Protocol protocol) {
        int r;
        initProtocol(protocol);
        protocol.setProductTypeTX(Data.substring(2, 2 + 2));
        protocol.setMeterID(Data.substring(4, 4 + 8));
    ...
    private static int MEAnalysisRxStr(String Data, Protocol protocol) {
        ...
        if (r == 1) {
            protocol.setSumHeat(Double.valueOf(heat));
        } else {
            protocol.setSumHeat(0);
            return r;
        }
        ...
        if (r == 1) {
            protocol.setTotal(Double.valueOf(total));
        } else {
            protocol.setTotal(0);
            return r;
        }
        ...
        if (r == 1) {
            protocol.setPower(Double.valueOf(power));
        } else {
            protocol.setPower(0);
            return r;
        }
        ...
        if (r == 1) {
            protocol.setFlowRate(Double.valueOf(flowrate));
        } else {
            protocol.setFlowRate(0);
            return r;
        }
        ...
        if (r == 1) {
            protocol.setT1InP(Double.valueOf(Temp1));
        } else {
            protocol.setT1InP(0);
            return r;
        }
        ...
        if (r == 1) {
            protocol.setT2InP(Double.valueOf(Temp2));
        } else {
            protocol.setT2InP(0);
            return r;
        }
        ...
        if (r == 1) {
            protocol.setWorkTimeInP(Integer.valueOf(WorkTime));
        } else {
            protocol.setWorkTimeInP(0);
            return r;
        }
    ...
    private static int ReportAnalysisJSMTRxStr(String Data, Protocol protocol) {
        int r;
        initProtocol(protocol);
        protocol.setProductTypeTX(Data.substring(2, 2 + 2));
        protocol.setProductTypeTX(Data.substring(2, 2 + 2));
...
//				   protocol.setSumOpenValveS("0");
    ...
    //解析收到的指令
    public static int analyzeData(String rx, Protocol protocol, SSumHeat ssumheat, MBUS mbus, ParameterProtocol parameterprotcol) {
        ...
        try {
            ...
            if (position_head_instruction >= 0) {
                ...
                //超声波水表
                if (head_instruction.equals("68") & productType.equals(ProductType.WATER_METER)) {
                    ...
                    if (rx.substring(position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2, position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2).equals("16")) {
                        ...
                        if (control_code1.equals("84") & control_code2.equals("A0") & control_code3.equals("17")) {
                            if (AnalysisUtils.checksum(data.substring(0, (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2 - 2), 0, data.substring((AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2 - 2, (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2)) == 1) {
                                r = ProductType.WATER_METER_OPEN_CLOSE_VALVE;
                            }
                        }
                        ...
                        if (control_code1.equals("81") & control_code2.equals("90") & (control_code3.equals("1F") || control_code3.equals("2F"))) {
                            if (AnalysisUtils.checksum(data.substring(0, (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2 - 2), 0, data.substring((AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2 - 2, (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2)) == 1) {
                                if (data.substring(20, 20 + 2).equals("16")) {
                                    if (analysisWaterMeterData(data, protocol) == 1) {
                                        r = ProductType.WATER_METER_READ_METER_DATA;
                                    }
                                }
                                if (data.substring(20, 20 + 2).equals("1B")) {
                                    if (analysisWaterMeterData1(data, protocol) == 1) {
                                        r = ProductType.WATER_METER_READ_METER_DATA;
                                    }
                                }
                            }
                ...
                //智能消火栓
                if (head_instruction.equals("68") & productType.equals(ProductType.HYDRANT)) {
                    ...
                    if (rx.substring(position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2, position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2).equals("16")) {
                        ...
                        if ((AnalysisUtils.checksum(data.substring(0, (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2 - 2), 0, data.substring((AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2 - 2, (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2)) == 1)) {
                            ...
                            if (control_code1.equals("81") & control_code2.equals("90") & control_code3.equals("1F")) {
                                if (analysisHydrantData(data, protocol) == 1) {
                ...
                if (head_instruction.equals("68") & productType.equals(ProductType.HEAT_METER)) {
                    ...
                    if (rx.substring(position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2, position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2).equals("16")) {
                        ...
                        if (control_code1.equals("81") & control_code2.equals("90") & (control_code3.equals("1F") || control_code3.equals("2F"))) {
                            if (AnalysisUtils.checksum(data.substring(0, (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2 - 2), 0, data.substring((AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2 - 2, (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2)) == 1) {
                                if (MVCAnalysisJSMTRxStr(data, protocol) == 1 || MCAnalysisJSMTRxStr(data, protocol) == 1) {
                ...
                //M-BUS主机
                if (head_instruction.equals("68") & productType.equals(ProductType.M_BUS)) {
                    length_tx = rx.substring(position_head_instruction + 20, position_head_instruction + 20 + 2);
                    if (rx.substring(position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2 - 2, position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2).equals("16")) {
                        data = rx.substring(position_head_instruction, position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 13) * 2);
                        control_code1 = data.substring(18, 18 + 2);
                ...
                if (head_instruction.equals("68") & productType.equals(ProductTypeS1) & head_instruction.equals(HeadS1)) {
                    ...
                    if (rx.substring(position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 6) * 2 - 2, position_head_instruction + (AnalysisUtils.HexS2ToInt(length_tx) + 6) * 2).equals("16")) {
                        ...
                        if (control_code1.equals("08") & control_code2.equals("72")) {
                            if (AnalysisUtils.checksum(data.substring(0, (AnalysisUtils.HexS2ToInt(length_tx) + 6) * 2 - 2 - 2), 4, data.substring((AnalysisUtils.HexS2ToInt(length_tx) + 6) * 2 - 2 - 2, (AnalysisUtils.HexS2ToInt(length_tx) + 6) * 2 - 2)) == 1) {
